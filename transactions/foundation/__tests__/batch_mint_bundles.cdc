...
transaction(rawTier: UInt8, rewardID: UInt32, quantity: UInt64) {
  ...
  prepare(cmAdmin: AuthAccount) {
    ...
    self.reservedTiersCollection = cmAdmin.borrow<&ChainmonstersFoundation.TiersCollection>(from: ChainmonstersFoundation.BundlesCollectionStoragePath) ?? panic("Tier collection missing")
  }
